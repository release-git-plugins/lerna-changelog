...
const namespace = '@release-git-plugins/lerna-changelog';
...
describe('@release-git-plugins/lerna-changelog', () => {
  ...
  test('throws if launchEditor is `true`, no $EDITOR present, and `editor` is not found on $PATH', async () => {
    ...
    try {
      ...
      expect(error.message).toEqual(
        `@release-git-plugins/lerna-changelog configured to launch your editor but no editor was found (tried $EDITOR and searching $PATH for \`editor\`).`
      );
    } finally {
      resetEDITOR();
      resetPATH();
